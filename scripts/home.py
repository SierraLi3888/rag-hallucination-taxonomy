...
def render_home(records, repo):
    ...
    short_titles = ['Information need', 'Retriever alignment & bias', 'Evidence granularity', 'Complementary evidence', 'Multi-hop dependencies', 'Evidence & hallucination', 'Assessment & priorities']
    full_titles = ['1 Representing the information need without changing the question', '2 Matching query representations to retrievers and their biases', '3 Finding the required evidence at the right granularity', '4 Selecting complementary evidence rather than redundant relevance', '5 Preserving dependencies through multi hop and iterative retrieval', '6 Distinguishing insufficient evidence from downstream hallucination', '7 Overall assessment and research priorities']
    chapter_labels = dict(zip(full_titles, short_titles))
    ...
    for i, area in enumerate(graph):
        ...
        if area['id'] == 'query-retrieval':
            for chapter in area['children']:
                if chapter['label'] in chapter_labels:
                    chapter['shortLabel'] = chapter_labels[chapter['label']]
